...
function getResultsHtml(results) {
  const output = results.map(item =>
    `<div class="search_item sep">` +
      `<div class="user">` +
        `<div class="user_avatar"><img src="avatar.jpg"></div>` +
        `<div class="user_text">` +
          `<div class="user_name">Duane Johnson</div>` +
          `<div class="user_infoline">` +
            `<div class="user_handle"><span>@</span>canadaduane</div>` +
            `<div class="search_time"><a href="canadaduane/status/${item.id_str}">${new Date(item.created_at).toLocaleString()}</a></div>` +
          `</div>` +
        `</div>` +
      `</div>` +
    ...
    .replace(/\.\.\/\.\.\/tweets_media\//g,'canadaduane/tweets_media/'));
